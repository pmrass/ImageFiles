classdef PMCZIMetaData
    ...
    methods % GETTERS: STITCHING
         ...
         function [Stitching ] = getFullStitchingStructure(obj)
                ...
                MyXShiftInput =             cell2mat(SummaryStart(2 : end, 1));
                Stitching.X =              obj.getStitchingStructureForColumn(MyXShiftInput);

                 MyYShiftInput =             cell2mat(SummaryStart(2 : end, 2));
                Stitching.Y =              obj.getStitchingStructureForColumn(MyYShiftInput);
         end
        ...
        function StitchingStructure = getStitchingStructureForColumn(obj, MyXShiftInput)


                XShifts =                   MyXShiftInput - min(MyXShiftInput);
                XSize =                     unique(MyXShiftInput);
                assert(length(XSize), 'Inconsistent image sizes not allowed.')

                 StitchingStructure.TotalSize =     max(XShifts) + XSize;
                 StitchingStructure.PanelSize =     XSize;
